...
import { firstStepContent } from "@/constants";
...
export default function FirstStep() {
  return (
    <section>
      ...
      <div
        className="md:px-12 px-6 pt-24 bg-cover bg-center bg-no-repeat w-full h-full min-h-[800px] md:grid md:grid-cols-2 flex flex-col-reverse"
        style={{ backgroundImage: `url(${firstStepContent.backgroundImage})` }}
      >
        <div
          className="bg-contain bg-bottom-left bg-no-repeat w-full h-full mx-auto col-span-1 min-h-[600px]"
          style={{
            backgroundImage: `url(${firstStepContent.foregroundImage})`,
          }}
        ></div>
        <div className="text-primary-foreground flex flex-col gap-12 justify-center">
          ...
          <div className="border-l w-1/2 h-48 pl-6 flex flex-col justify-between">
            <p className="leading-[clamp(1rem,1.5vw,2rem)] text-[clamp(1rem,1.5vw,2rem)] tracking-tighter">
              {firstStepContent.description}
            </p>
            {/*<span className="uppercase text-xs text-primary-foreground/50">
              <p>{firstStepContent.personName}</p>
              <p>{firstStepContent.personRole}</p>
            </span>*/}
          </div>
        </div>
      </div>
